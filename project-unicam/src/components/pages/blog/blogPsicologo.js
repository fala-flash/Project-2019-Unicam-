...
class BlogPsicologo extends Component {
  ...
  setEmoji(punteggio) {
    if (punteggio === 0) {
      return (
        <FaCircle size={35} style={{ color: "#grey", borderRadius: "50%" }} />
      );
    } else if (punteggio === -1) {
      return (
        <FaCircle size={35} style={{ color: "#f2ea15", borderRadius: "50%" }} />
      );
    } else if (punteggio === -2) {
      return (
        <FaCircle size={35} style={{ color: "orange", borderRadius: "50%" }} />
      );
    } else if (punteggio <= -3) {
      return (
        <FaCircle size={35} style={{ color: "#a80000", borderRadius: "50%" }} />
      );
    } else if (punteggio === 1) {
      return (
        <FaCircle size={35} style={{ color: "#7be85a", borderRadius: "50%" }} />
      );
    } else if (punteggio === 2) {
      return (
        <FaCircle size={35} style={{ color: "#49d81e", borderRadius: "50%" }} />
      );
    } else if (punteggio >= 3) {
      return (
        <FaCircle size={35} style={{ color: "#28af00", borderRadius: "50%" }} />
      );
    }
  }
    ...
    segnalazione.once("value", snap => {
      snap.forEach(child => {
        this.setState({
          ...
          punteggioAnalisi: this.state.punteggioAnalisi.concat([
            child.val().punteggioAnalisi
          ]),
          valutazioneAnalisi: this.state.valutazioneAnalisi.concat([
            child.val().valutazioneAnalisi
          ]),
        ...
        >
          ...
          {this.state.fotoUtente[this.state.indexSegnalazione] != null ? (
            <img
              ...
              alt="fotoutente"
              src={this.state.fotoUtente[this.state.indexSegnalazione]}
            />
          ) : (
            <h3 style={{ color: "rgb(10,126,126)" }}>
              {" "}
              La foto non è disponibile
            </h3>
          )}
        </Modal.Body>
        <Modal.Footer
              ...
              >
                <Card.Header>
                  <Card.Title>
                    ...
                    <br />
                    <br />
                    {this.setEmoji(this.state.punteggioAnalisi[index])}{" "}
